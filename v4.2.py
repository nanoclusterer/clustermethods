...
def kmeans():
    ...
    try:
        ...
        # Anzahl der Iterationen anzeigen
        iterations_label.configure(text=f"Benötigte Iterationen: {kmeans.n_iter_}")
...
def kMeans_success(df, featureColumns, canvasILS, canvasRmin):
    ...
    axILS.scatter(df_pca[featureColumns[0]].values, df_pca[featureColumns[1]].values, 
                               s=50, c=df_pca['kMean_optimal'].values, cmap='viridis')
    ...
    axILS.scatter(centroids[:, 0], centroids[:, 1], s=200, c='red', label='Centroids')
    axILS.legend()
