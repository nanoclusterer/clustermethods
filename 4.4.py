...
def dbscan():
    ...
    try:
        ...
        for label in set(labels):
            ...
            axDBSCAN.scatter(X[label_mask, 0], X[label_mask, 1], s=10, label=f"Cluster {label}")
...
def show_empty_plot_rmin():
    ...
    axRmin.axis("off")
    axRmin.text(0.5, 0.5, "R-min", horizontalalignment='center', verticalalignment='center', fontsize=12, transform=axData.transAxes)
    canvasRmin.draw()
...
def show_empty_plot_ils():
    ...
    axILS.axis("off")
    axILS.text(0.5, 0.5, "ILS", horizontalalignment='center', verticalalignment='center', fontsize=12, transform=axData.transAxes)
    canvasILS.draw()
...
def determine_optimal_clusters(minR):
    peaks, _ = find_peaks(minR, distance=10, prominence=0.15)  # Distance und prominence für Spikes anpassen
...
def kMeans_success(df, featureColumns, canvasILS, canvasRmin):
    ...
    axILS.scatter(df_pca[featureColumns[0]].values, df_pca[featureColumns[1]].values, 
                               s=10, c=df_pca['kMean_optimal'].values, cmap='viridis')
...
fensterlinks = ctk.CTkFrame(root, fg_color=frame_color)
fensterlinks.grid(row=0, column=0, padx=(180, 6), pady=6, sticky = 'ne')

fensterrechts = ctk.CTkFrame(root, fg_color=frame_color)
fensterrechts.grid(row=0, column=1, padx=6, pady=6, sticky = 'nw')
...
data_input_frame = ctk.CTkFrame(fensterlinks, fg_color=frame_color)
data_input_frame.grid(row=0, column=0, padx=6, pady=6, sticky = 'ne')
...
data_input_label.grid(row=0, column=0, padx=10, pady=0, sticky = 'w')
...
kmeans_frame = ctk.CTkFrame(fensterrechts, fg_color=frame_color)
kmeans_frame.grid(row=0, column=1, padx=6, pady=6, sticky = 'nw')
...
# Label zur Anzeige der Iterationen
iterations_label = ctk.CTkLabel(kmeans_frame, text="Benötigte Iterationen: ", font=label_font)
iterations_label.grid(row=4, column=2, padx=10, pady=10, sticky = 'w')
...
dbscan_frame = ctk.CTkFrame(fensterlinks, fg_color=frame_color)
dbscan_frame.grid(row=1, column=0, padx=6, pady=6, sticky = 'ne')
dbscan_label = ctk.CTkLabel(dbscan_frame, text = "DBSCAN", font=headline_font)
dbscan_label.grid(row=0, column=0, padx=10, pady=0, sticky = 'w')
...
ils_frame = ctk.CTkFrame(fensterrechts, fg_color=frame_color)
ils_frame.grid(row=1, column=1, padx=6, pady=6, sticky = 'nw')
